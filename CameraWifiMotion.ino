...
void handleImages(){
  ...
  // action any buttons presses or url parameters
    ...
    // if 'small image' button was pressed
      String imagePrefixText = "";        
      if (server.hasArg("smallver")) {
        imagePrefixText = "10";   // added to front of required image to show (i.e. change 1 to 101)
      }
  ...
  // button to show small version of image
    message += "<BR><input name='smallver' value='Show low resolution first image' type='submit'>\n";
    
  // insert image in to html 
    message += "<BR><img id='img' alt='Camera Image' onerror='QpageRefresh();' width='" + 
       ImageWidthSetting + "%' src='/img?pic=" + imagePrefixText + String(ImageToShow) + "'>\n";   

  // javascript to refresh the image if it fails to load (bug fix as it often rejects the request otherwise)
    message +=  "<script type='text/javascript'>\n"
                "  function QpageRefresh() {\n"
                "    setTimeout(function(){ document.getElementById('img').src='/img?pic=" + imagePrefixText + String(ImageToShow) + "'; }, " + JavaRefreshTime + ");\n"
                "  }\n"
                "</script>\n";
